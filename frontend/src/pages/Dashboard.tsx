...
function ResultCards({ files, selectable = false, selected = new Set<string>(), onToggle }: {
  ...
  return <div className="media-grid">{files.map((file) => (
    <article className="media-card" key={file.checksum}>
      ...
      <div className="media-meta">
        {selectable && <input aria-label={`select ${file.checksum}`} type="checkbox" checked={selected.has(file.checksum)} onChange={() => onToggle?.(file.checksum)} />}
        <div><strong>{file.filename || `${file.file_type || 'media'} 文件`}</strong><small>{file.checksum.slice(0, 12)}…</small></div>
      </div>
      <p className="tags">{tagText(file.tags)}</p>
...
function QueryPanel() {
  ...
  const queryThumbnail = async () => {
    setBusy(true); setMessage('');
    try {
      const found = await api.queryByThumbnail(thumbnail);
      setResults([{ ...found, url: found.url || found.full_url }]);
    }
  ...
  return <section className="panel">
    ...
    <div className="query-grid">
      <form onSubmit={(e) => { e.preventDefault(); void queryTags(); }}>
        <label>标签与最小数量</label><input value={tagInput} onChange={(e) => setTagInput(e.target.value)} placeholder="dingo:2, cat:1" />
        <button disabled={busy}>查询标签</button>
      </form>
      <form onSubmit={(e) => { e.preventDefault(); void queryThumbnail(); }}>
        <label>缩略图 URL</label><input value={thumbnail} onChange={(e) => setThumbnail(e.target.value)} placeholder="https://.../thumb.jpg" required />
        <button disabled={busy}>反查原图</button>
      </form>
      <div className="file-query"><label>按文件中的物种查询</label><label className="file-button">选择查询文件<input hidden type="file" accept="image/*,video/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) void queryFile(file); }} /></label></div>
    </div>
...
function ManagementPanel({ selected, onChanged }: { selected: string[]; onChanged: () => void }) {
  ...
  return <section className="panel">
    ...
    <div className="management-grid">
      <div><label>标签（逗号分隔）</label><input value={tags} onChange={(e) => setTags(e.target.value)} />
        <div className="button-row"><button onClick={() => void mutate(1)}>添加标签</button><button className="secondary" onClick={() => void mutate(0)}>删除标签</button><button className="danger" onClick={() => void remove()}>删除文件</button></div></div>
      <form onSubmit={subscribe}><label>邮件通知</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@example.com" required />
        <input value={watchTags} onChange={(e) => setWatchTags(e.target.value)} placeholder="Sus_scrofa, dingo" required /><button>订阅标签</button></form>
    </div>
...
export function Dashboard() {
  ...
  return <div className="dash">
    ...
    <main className="dashboard-grid">
      ...
      <section className="panel library-panel"><div className="panel-heading"><div><h2>媒体库</h2><p>缩略图来自私有 OSS 短期签名 URL。</p></div><button className="secondary" onClick={() => void refresh()}>{loading ? '刷新中…' : '刷新'}</button></div>
        {error && <p className="error">{error}</p>}
        <ResultCards files={files} selectable selected={selected} onToggle={(id) => setSelected((current) => { const next = new Set(current); next.has(id) ? next.delete(id) : next.add(id); return next; })} />
      </section>
      <QueryPanel />
