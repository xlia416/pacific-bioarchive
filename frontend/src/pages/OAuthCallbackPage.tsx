...
export function OAuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (oauthError || !code) { setError(oauthError || '缺少 OAuth authorization code'); return; }
    completeOAuthCallback(code, params.get('state')).then(() => navigate('/', { replace: true }))
      .catch((err) => setError(err?.message || 'Google 登录失败'));
  }, [navigate]);
  return <div className="splash">{error ? <span className="error">{error}</span> : '正在完成 Google 登录…'}</div>;
}
